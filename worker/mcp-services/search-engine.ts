...
import type { AdditionalUrl, SearchOptions, SearchResult } from "../mcp-types/index.js";
...
export class SearchEngine {
	...
	private async hybridSearchWithReranker(
		...
		try {
			...
			finalResults = processedResults.slice(0, resultCount).map((processed, index) => ({
				id: processed.id,
				url: processed.url,
				title: processed.title,
				content: processed.content,
	...
	private mergeByTitle(results: SearchResult[]): ProcessedResult[] {
		...
		return Array.from(titleGroups.entries()).map(([title, group]) => {
			...
			const isCompleteDocument =
				chunkIndices.length === primary.total_chunks && chunkIndices.every((idx, i) => idx === i);
